...
def support(request):
    ...
    #조원별 시간과 댓글크롤링 갯수
    #sohyun
    cursor.execute("select mem_name, count(comment) as count, substr(commemt_get_time,1, 2) as hour from board_comment1 where mem_name = 'SoHyun' group by substr(commemt_get_time,1, 2)")
    row = cursor.fetchall()
    ...
    for j in colname:
        col.append(j[0])
    s_df = pd.DataFrame(row, columns=col)
    
    #sanga
    cursor.execute("select mem_name, count(comment) as count, substr(commemt_get_time,1, 2) as hour from board_comment1 where mem_name = 'SangA' group by substr(commemt_get_time,1, 2)")
    row = cursor.fetchall()
    colname = cursor.description
    ...
    for j in colname:
        col.append(j[0])
    a_df = pd.DataFrame(row, columns=col)
    
    #minhee
    cursor.execute("select mem_name, count(comment) as count, substr(commemt_get_time,1, 2) as hour from board_comment1 where mem_name = 'MinHee' group by substr(commemt_get_time,1, 2)")
    row = cursor.fetchall()
    colname = cursor.description
    ...
    for j in colname:
        col.append(j[0])
    m_df = pd.DataFrame(row, columns=col)
    
    #jihyun
    cursor.execute("select mem_name, count(comment) as count, substr(commemt_get_time,1, 2) as hour from board_comment1 where mem_name = 'JiHyun' group by substr(commemt_get_time,1, 2)")
    row = cursor.fetchall()
    colname = cursor.description
    ...
    for j in colname:
        col.append(j[0])
    j_df = pd.DataFrame(row, columns=col)
    
    # for i in a_df.values:
    #     print(i[0])
    
    label = []
    label.append(s_df.loc[0, 'mem_name'])
    label.append(a_df.loc[0, 'mem_name'])
    label.append(m_df.loc[0, 'mem_name'])
    label.append(j_df.loc[0, 'mem_name'])
    ...
    context = {"con1": s1, "con2": s2, "con3": s3, "con4": s4, "com":com0, "com1":com1, "com2":com2, "com3":com3, "cap":cap0, "cap1":cap1, "cap2":cap2, "cap3":cap3, 
            ...
            's_df': s_df, 'a_df':a_df, 'm_df':m_df, 'j_df':j_df, 'label':label}
    cursor.close()
    conn.close()    
